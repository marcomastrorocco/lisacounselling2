...
const seedPages = [
  ...
  {id: 'services', label: 'Services', file: 'services/index.html', path: '/services/'},
  {id: 'ndis-counselling', label: 'NDIS counselling', file: 'ndis-counselling/index.html', path: '/ndis-counselling/'},
  {id: 'ndis-therapeutic-support', label: 'NDIS Therapeutic Support', file: 'ndis-therapeutic-support/index.html', path: '/ndis-therapeutic-support/'},
  {id: 'eap-counselling', label: 'EAP Counselling', file: 'eap-counselling/index.html', path: '/eap-counselling/'},
...
function updateNavigation(shell, action, {label, path: href}) {
  ...
  const headerAnchor = /<a class="button" href="\{\{HALAXY_URL\}\}">Book an [Aa]ppointment<\/a>/
  const footerAnchor = '<a href="/privacy/">Privacy Policy</a>'
  const link = `<a href="${href}">${label}</a>`

  if (action === 'add') {
    const anchor = shell.match(headerAnchor)
    if (shell.includes(link) || !anchor || !shell.includes(footerAnchor)) return null
    return shell.replace(headerAnchor, link + anchor[0]).replace(footerAnchor, link + footerAnchor)
  }
